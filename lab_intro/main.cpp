...
PNG brighten(PNG original, int amount)
{
    ...
    for (int yi = 0; yi < original.height(); yi++)
    {
        ...
        {
            // Your code here
            int red_val = original(xi, yi)->red + amount;
            if (red_val > 255)
            {
                red_val = 255;
            }
            original(xi, yi)->red = red_val;

            int green_val = original(xi, yi)->green + amount;
            if (green_val > 255)
            {
                green_val = 255;
            }
            original(xi, yi)->green = green_val;

            int blue_val = original(xi, yi)->blue + amount;
            if (blue_val > 255)
            {
                blue_val = 255;
            }
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    for (int yi = 0; yi < min(firstImage.height(), secondImage.height()); yi++)
    {
        ...
        {
            // Your code here
            int red_1 = firstImage(xi, yi)->red;
            int red_2 = secondImage(xi, yi)->red;
            firstImage(xi, yi)->red = (red_1 + red_2) / 2;

            int green_1 = firstImage(xi, yi)->green;
            int green_2 = secondImage(xi, yi)->green;
            firstImage(xi, yi)->green = (green_1 + green_2) / 2;

            int blue_1 = firstImage(xi, yi)->blue;
